...
import { Moon, Tag, ExternalLink } from "lucide-react";
...
export default function AdminUmrahPackagesPage() {
  ...
  const loadUmrahBookings = async () => {
    ...
    try {
      ...
      if (json?.data) {
        const filtered = json.data.filter((b: any) => (b.type || "").toLowerCase() === "umrah");
        setUmrahBookings(filtered);
      }
    } catch (err) {
  ...
  useEffect(() => {
    loadUmrahBookings();
    const handleFocus = () => loadUmrahBookings();
  ...
  const handleUpdateStatus = async (id: string, newStatus: string) => {
    ...
    try {
      ...
      console.error("Failed to update Umrah booking status", err);
      loadUmrahBookings();
    }
  };
  ...
  const filteredBookings = umrahBookings.filter((b) => {
    const matchesStatus = filterStatus === "ALL" || b.status === filterStatus;
    const q = searchQuery.toLowerCase();
    const pilgrimName = b.passengers?.[0] ? `${b.passengers[0].firstName} ${b.passengers[0].lastName}` : b.user?.name || "";
    const pnr = b.pnr || "";
    const packageTitle = b.destination || "";
    const phone = b.passengers?.[0]?.phone || b.user?.phone || "";
    const email = b.passengers?.[0]?.email || b.user?.email || "";
    const matchesSearch =
      pilgrimName.toLowerCase().includes(q) ||
      pnr.toLowerCase().includes(q) ||
      packageTitle.toLowerCase().includes(q) ||
      phone.includes(q) ||
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            View customer Umrah reservations and manage live package listings.
          </p>
        </div>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              activeTab === "reservations"
                ? "bg-white text-[#FF8B3D] shadow-xs dark:bg-gray-900 dark:text-[#FF8B3D]"
                : "text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white"
            }`}
          >
            <Moon className="w-4 h-4 text-[#FF8B3D] fill-[#FF8B3D]/20" />
            <span>Customer Umrah Reservations ({umrahBookings.length})</span>
          </button>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="relative flex-1 max-w-md">
              <input
                type="text"
                placeholder="Search by Pilgrim Name, Email, Phone, or PNR..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pl-10 pr-4 text-xs text-gray-800 outline-none focus:border-emerald-500 dark:border-gray-800 dark:bg-gray-900 dark:text-white"
              />
              <svg
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative sm:w-56">
                <button
                  ...
                  className="dropdown-toggle h-10 w-full inline-flex items-center justify-between gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2 text-xs font-medium text-gray-700 shadow-xs focus:border-emerald-300 dark:border-gray-800 dark:bg-gray-900 dark:text-white/90 cursor-pointer"
                >
              ...
              >
                <svg className={`w-3.5 h-3.5 ${isRefreshing ? "animate-spin text-emerald-500" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
                <span>{isRefreshing ? "Refreshing..." : "Refresh Reservations"}</span>
              </button>
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
                    <th className="px-5 py-4 font-medium text-gray-500 text-xs dark:text-gray-400 whitespace-nowrap">Lead Pilgrim</th>
                    <th className="px-5 py-4 font-medium text-gray-500 text-xs dark:text-gray-400 whitespace-nowrap">Booking Reference / PNR</th>
                    <th className="px-5 py-4 font-medium text-gray-500 text-xs dark:text-gray-400 whitespace-nowrap">Umrah Package</th>
                    <th className="px-5 py-4 font-medium text-gray-500 text-xs dark:text-gray-400 whitespace-nowrap">Departure City &amp; Date</th>
                    <th className="px-5 py-4 font-medium text-gray-500 text-xs dark:text-gray-400 whitespace-nowrap">Total Amount</th>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {paginatedBookings.length > 0 ? (
                    paginatedBookings.map((b) => {
                      const pilgrim = b.passengers?.[0] || {};
                      const pilgrimName = pilgrim.firstName ? `${pilgrim.firstName} ${pilgrim.lastName}` : b.user?.name || "Lead Pilgrim";
                      const pilgrimPhone = pilgrim.phone || b.user?.phone || "N/A";
                      const pilgrimEmail = pilgrim.email || b.user?.email || "";
                      const travelDateStr = new Date(b.departureDate).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" });
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-3">
                              <div className={`flex items-center justify-center w-10 h-10 rounded-full ${getAvatarColor(pilgrimName)} shrink-0`}>
                                <span className="text-sm font-semibold text-white">
                                  {getInitials(pilgrimName)}
                                </span>
                              </div>
                              <div>
                                ...
                                <span className="text-gray-500 text-xs dark:text-gray-400 block font-mono">
                                  {pilgrimPhone} {pilgrimEmail ? `• ${pilgrimEmail}` : ""}
                                </span>
                              </div>
                            </div>
                          </td>
                          ...
                          {/* PNR Code */}
                          <td className="px-5 py-4 whitespace-nowrap">
                            <span className="font-mono font-semibold text-[#FF8B3D] text-xs">
                              {b.pnr}
                            </span>
                          ...
                          <td className="px-5 py-4 whitespace-nowrap">
                            <span className="font-mono font-bold text-gray-800 dark:text-gray-200 text-xs block">
                              {b.currency === "EUR" ? "€" : "$"}{b.totalAmount?.toLocaleString()}
                            </span>
                            <span className="text-[10px] text-gray-400 uppercase">
                              {b.payment?.gateway || "Consultation"}
                            </span>
                          </td>
                          ...
                          <td className="px-5 py-4 whitespace-nowrap text-right">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <button
                                title="View Umrah Reservation Details"
                                onClick={() => setViewingBooking(b)}
                                className="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-gray-200 bg-white text-gray-500 hover:bg-emerald-50 hover:border-emerald-200 hover:text-emerald-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-emerald-500/10 dark:hover:text-emerald-400 transition-colors duration-150 cursor-pointer"
                              >
                    ...
                    <tr>
                      <td colSpan={7} className="px-5 py-10 text-center text-gray-500 dark:text-gray-400 text-xs">
                        No Umrah package reservations found matching your criteria.
                      </td>
                    </tr>
                  )}
                </tbody>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                  <button
                    ...
                    className={`inline-flex items-center justify-center w-8 h-8 rounded-lg border text-xs font-semibold transition-colors duration-150 cursor-pointer ${
                      p === page
                        ? "border-[#FF8B3D] bg-[#FF8B3D] text-white shadow-xs"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-3xl rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-900 border border-gray-200 dark:border-gray-800 max-h-[92vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-gray-100 pb-4 dark:border-gray-800">
              <div>
                <span className="text-[10px] uppercase font-bold tracking-wider text-gray-400 block">PNR Code</span>
                <span className="font-mono font-bold text-[#FF8B3D] text-sm">
                  #{viewingBooking.pnr}
                </span>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white font-outfit mt-1">
                  {viewingBooking.passengers?.[0] ? `${viewingBooking.passengers[0].firstName} ${viewingBooking.passengers[0].lastName}` : viewingBooking.user?.name || "Pilgrim"}&apos;s Umrah Package Reservation
                </h3>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5 grid grid-cols-2 gap-y-4 gap-x-6 text-xs bg-gray-50/70 dark:bg-gray-800/40 p-4 rounded-xl border border-gray-100 dark:border-gray-800">
              <div>
                <span className="text-gray-400 block font-medium">Umrah Package</span>
                <span className="font-bold text-gray-900 dark:text-gray-100">{viewingBooking.destination}</span>
              </div>
              <div>
                <span className="text-gray-400 block font-medium">Package Details / Duration</span>
                <span className="font-bold text-gray-900 dark:text-gray-100">{viewingBooking.flightNumber || "Umrah Package"}</span>
              </div>
              <div>
                <span className="text-gray-400 block font-medium">Lead Pilgrim Name</span>
                <span className="font-semibold text-gray-800 dark:text-gray-200">
                  {viewingBooking.passengers?.[0] ? `${viewingBooking.passengers[0].firstName} ${viewingBooking.passengers[0].lastName}` : viewingBooking.user?.name || "Lead Pilgrim"}
                </span>
              </div>
              <div>
              ...
              <div>
                <span className="text-gray-400 block font-medium">Departure City</span>
                <span className="font-bold font-mono text-emerald-600 dark:text-emerald-400">{viewingBooking.origin}</span>
              </div>
              <div>
              ...
              <div>
                <span className="text-gray-400 block font-medium">Total Package Price</span>
                <span className="font-extrabold text-emerald-600 dark:text-emerald-400 text-sm">
                  {viewingBooking.currency === "EUR" ? "€" : "$"}{viewingBooking.totalAmount?.toLocaleString()}
                </span>
              </div>
              <div>
                ...
                <span className="font-semibold text-gray-800 dark:text-gray-200 uppercase">
                  {viewingBooking.payment?.gateway || "Consultation"}
                </span>
              </div>
            </div>
            ...
            <div className="mt-6 flex items-center justify-between">
              {viewingBooking.passengers?.[0]?.phone ? (
                <a
                  href={`https://wa.me/${viewingBooking.passengers[0].phone.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(`Salam ${viewingBooking.passengers[0].firstName}! Regarding your Umrah Package Reservation ${viewingBooking.pnr} for ${viewingBooking.destination}...`)}`}
                  target="_blank"
              ...
              <button
                type="button"
                onClick={() => setViewingBooking(null)}
                className="rounded-xl bg-emerald-600 px-6 py-2.5 text-xs font-bold text-white shadow-theme-xs hover:bg-emerald-700 transition-colors cursor-pointer font-outfit"
              >
                Close Preview
              </button>
            </div>
